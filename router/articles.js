...
//que l'on peut faire à notre serveur sur la ressource articles

const express = require("express");
const mongoose = require("mongoose");
const router = express.Router(); // créer note router puis l'éxporter dans /app.js
// schema des données => colonnes
const { Articles, schema } = require("../model/articles"); // importer le shema depuis le /model. articles.js

// ============== récupérer toute la liste des articles ====================================
...
router.post("/", async function(req, res) {
    ...
    const resultat = await articles.save(); // asychrone => attendre que Mongo écrive 
    res.send(resultat);
});
